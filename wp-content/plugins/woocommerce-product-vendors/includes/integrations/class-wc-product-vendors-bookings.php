...
class WC_Product_Vendors_Bookings {
	...
	public function __construct() {
		...
		add_action( 'admin_init', array( $this, 'filter_page_access' ) );
	...
	public function filter_page_access() {
		...
		if ( 'wc_booking' === $post_type ) {			
			...
			$post_id = get_post_meta( $post_id, '_booking_product_id', true );
		}

		if (
			preg_match( '/post.php/i', $url ) &&
			$post_id &&
			isset( $_GET['action'] ) &&
			'edit' === $_GET['action'] &&
			! WC_Product_Vendors_Utils::can_user_manage_product( $vendor_id, $post_id )
		) {
			/*
			 * If they don't have access to manage
			 * this booking, redirect them back
			 * to the listing.
			 */
